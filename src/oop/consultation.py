...
# Controller layer that connects UserSession (OOP state) with PrologBridge (inference).
# Owns the full consultation flow: start -> ask -> answer -> diagnose -> end.
...
class Consultation:
    ...
    def __init__(self, session: UserSession, bridge: PrologBridge,
        ...
        # reasoning thread blocks on this until intake is done
    ...
    def preload_symptoms(self, raw_text: str) -> dict:
        # sends free text through Lisp and asserts extracted symptoms into Prolog
        if not self._lisp:
        ...
        for s in to_assert:
            list(self._bridge._prolog.query(f"assertz(diagnosis_rules:symptom({s}))"))
            list(self._bridge._prolog.query(f"assertz(diagnosis_rules:asked({s}))"))
            self._preloaded.append(s)

        for s in to_deny:
            list(self._bridge._prolog.query(f"assertz(diagnosis_rules:denied({s}))"))
            list(self._bridge._prolog.query(f"assertz(diagnosis_rules:asked({s}))"))
    ...
    def answer(self, response: bool) -> None:
        if not self._session.is_active:
            return  # session already ended, ignore late answers
        with self._lock:
    ...
    def _reasoning_loop(self) -> None:
        try:
            ...
            while self._session.is_active:
                ...
                action = self._bridge.step(self._current_symptom, response)

                # skip symptoms already handled during intake
                ...
                # if only one candidate left with enough confidence, exit early
    ...
    def _get_next_skipping_preloaded(self) -> dict:
        ...
        for _ in range(30):
            results = list(self._bridge._prolog.query("next_question(S), symptom_question(S, Q)"))
            if not results:
            ...
            try:
                list(self._bridge._prolog.query(f"assertz(diagnosis_rules:asked({sym}))"))
            except Exception:
                pass
